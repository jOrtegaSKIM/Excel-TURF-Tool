# TODO: Consider picking instead of matrix mult
# - remove get.dummies
# - edit turf.iter
# - edit run.turf
# - edit stepwise.turf
...
turf.options <- function(df){
  top <- 10
  sigtest <- 0.1
  methodology <- "Unspoken"
  none <- TRUE
  exponent <- 1
  calculation <- "SoP"
  # kpi = Preference Share | Revenue | Profit
  kpi <- df %>% 
    filter(Option=="KPI") %>% pull(Value)
  # total volume of the market
  total_volume <- df %>% 
    filter(Option=="Volume") %>% pull(Value)
  # use_bucks = FALSE | TRUE
  use_bucks <- df %>% 
    filter(Option=="Bucketing") %>% pull(Value)
  # use_prohs = FALSE | TRUE
  use_prohs <- df %>% 
    filter(Option=="Prohibitions") %>% pull(Value)
  options <- list(
    top=top, sigtest=sigtest,
    methodology=methodology, none=none, 
    exponent=exponent, calculation=calculation,
    kpi=kpi, total_volume=as.numeric(total_volume),
...
turf.inputs <- function(df){
  ...
  lst_comp <- df %>% filter(Client == "No") %>% pull(Item)
  
  df <- df %>% filter(Client == "Yes")
  lst_fixed <- df %>% filter(Fixed == "Yes") %>% pull(Item)
  
  df <- df %>% filter(Fixed == "No") |>
    mutate(Bucket = as.numeric(as.factor(Bucket)))
  
  n_bucks <- n_distinct(df$Bucket)
  n_bucks <- if (n_bucks == 1) 0 else n_bucks
  bucks <- list()
  for (i in seq_len(n_bucks)){
    buck <- df %>% filter(Bucket == i) %>% pull(Item)
    bucks[[i]] <- buck
  }
  
  inputs <- list(
    n=n, item_wei=item_wei,
    dist=NA, size=NA,
    price=df$Price,
    cost=df$Cost,
    lst_fixed=lst_fixed,
...
get.combinations <- function(k,control){
  if (control$n_bucks == 0 | !control$use_bucks){
    ...
    partition <- blockparts(control$buck_lims,k)
    nck <- data.frame()
...
turf.iter <- function(i,nck_bin,utils,control,ttest.data=FALSE){
  ...
  if (control$calculation == "SoP") {
    ...
    #if (control$methodology == "MaxDiff") {
    #  case_max  <- rowMax(mapply(`*`,utils[,-(n+1)],control$item_wei[-(n+1)]))
    #} else if (control$methodology != "MaxDiff") {
